...
export async function getCommuneStats(codeCommune) {
  ...
  const sources = voies.map(voie => voie.sources)
  const destinations = voies.map(voie => voie.destination)

  return {
    sources: getCount(sources),
    destinations: getCount(destinations),
    citizensPerAddress: round(population / addresses, 2)
  }
}
