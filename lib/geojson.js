...
import {getStatus, getName, hasPosition, getNumeroPosition, getPosition, getNumeroPositions} from './bal/item'
...
function positionToFeature(position, properties) {
  const {_id, type, dateMAJ} = position
  const coords = getCoordinates(position)
  const props = {...properties}

  if (properties && properties.source) {
    props.source = properties.source[0]
  }

  return point(coords, {_id, type, dateMAJ, ...props})
}
...
export function positionsToGeoJson(positions, properties) {
  return {
    type: 'FeatureCollection',
    features: positions.map(position => positionToFeature(position, properties))
  }
}
...
export function numeroPositionsToGeoJson(numero) {
  const positions = getNumeroPositions(numero)
  return {
    type: 'FeatureCollection',
    features: positions.map(position => positionToFeature(position, numero))
  }
}
