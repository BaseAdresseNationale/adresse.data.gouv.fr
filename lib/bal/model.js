...
function createRow(obj) {
  const row = {
    ...
    ...splitNumeroComplet(obj.numeroComplet),
  ...
  tree.communes.forEach(commune => {
    ...
    commune.voies.forEach(voie => {
      ...
      if (voie.numeros.length > 0) {
        ...
        rows.push(createRow({...voieCtx, numeroComplet: '99999', position: voie.position}))
      } else {
        rows.push(createRow({...voieCtx, numeroComplet: '99999'}))
      }
    })
  })
  return rows
