...
async function createUser(req, res) {
  ...
  const mailOptions = {
    ...
    text: `Bonjour ${user.name}, merci de valider votre adresse : ${process.env.SITE_URL}/api-users/confirmation/${token.token}`
  }
...
async function renewEmailToken(req, res) {
...
async function renewPasswordToken(req, res) {
  ...
  if (user && !user.isDeleted) {
    ...
    const mailOptions = {
      ...
      text: `Bonjour ${user.name}, pour modifier votre mot de passe, merci de cliquer sur ce lien : ${process.env.SITE_URL}/api-users/renew-password?token=${token.token}`
    }
