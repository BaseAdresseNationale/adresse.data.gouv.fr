...
if (!publicRuntimeConfig.NEXT_PUBLIC_BAL_ADMIN_API_URL) {
...
export async function getOnePartenairesDeLaCharte(id: string): Promise<PartenaireDeLaChartType> {
  const url = new URL(`${publicRuntimeConfig.NEXT_PUBLIC_BAL_ADMIN_API_URL}/partenaires-de-la-charte/${id}`)
...
export async function getPartenairesDeLaCharte(queryObject: PartenairesDeLaCharteQuery, page: number = 1, limit: number = DEFAULT_PARTENAIRES_DE_LA_CHARTE_LIMIT): Promise<{ total: number, totalCommunes: number, totalOrganismes: number, totalEntreprises: number, data: PartenaireDeLaChartType[] }> {
  const url = new URL(`${publicRuntimeConfig.NEXT_PUBLIC_BAL_ADMIN_API_URL}/partenaires-de-la-charte/paginated`)
...
export async function getPartenairesDeLaCharteServices(): Promise<string[]> {
  return customFetch(`${publicRuntimeConfig.NEXT_PUBLIC_BAL_ADMIN_API_URL}/partenaires-de-la-charte/services`)
}

export async function getRandomPartenairesDeLaCharte(limit: number) {
  const url = new URL(`${publicRuntimeConfig.NEXT_PUBLIC_BAL_ADMIN_API_URL}/partenaires-de-la-charte/random`)
  if (limit) {
    url.searchParams.append('limit', limit.toString())
  }
...
export async function candidateToPartenairesDeLaCharte(candidacy: CandidatePartenaireDeLaCharteType) {
  const request = `${publicRuntimeConfig.NEXT_PUBLIC_BAL_ADMIN_API_URL}/partenaires-de-la-charte/candidate`
...
export async function getBalEvents(): Promise<EventType[]> {
  try {
    const response = await fetch(`${publicRuntimeConfig.NEXT_PUBLIC_BAL_ADMIN_API_URL}/events`)
    if (!response.ok) {
