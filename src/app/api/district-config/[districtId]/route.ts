...
export async function GET(_req: NextRequest, { params }: { params: { districtId: string } }) {
  ...
  if (GEO_URL) {
    ...
    if (userSiren) {
      ...
      mayExposeFullConfig = resolved.status === 'ok' && resolved.siren === userSiren
    }
  }

  const config = mayExposeFullConfig && fullConfig && typeof fullConfig === 'object'
    ? fullConfig as BANConfig
    : toPublicDistrictConfig(fullConfig as Partial<BANConfig>)
