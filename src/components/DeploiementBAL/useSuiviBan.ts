...
function normalizeCommuneCode(code: unknown): string {
  const raw = String(code || '').trim().toUpperCase()
  if (/^\d+$/.test(raw)) {
...
export function useSuiviBan({ selectedTab }: UseSuiviBanOptions) {
  ...
  const handleCommuneClick = useCallback(async (commune: any) => {
    const communeCode = normalizeCommuneCode(commune?.code)
    const normalizedCommune = { ...commune, code: communeCode }
    setSuiviBanSelectedCommune(normalizedCommune)
    let enriched = normalizedCommune
    let coordinates = getCommuneCoordinates(enriched)

    if (!coordinates && communeCode) {
      try {
        const response = await fetch(`${SUIVI_BAN_API}/commune/${encodeURIComponent(communeCode)}`)
        if (response.ok) {
          const full = await response.json()
          if (full && typeof full === 'object') {
            enriched = { ...full, ...normalizedCommune }
        ...
        console.warn('Commune detail load error:', communeCode, e)
      }
    }
  ...
  const handleMapClick = useCallback((e: any) => {
    ...
    if (communeFeature?.properties) {
      const clickedPointFallback = e?.lngLat
        ? { lon: e.lngLat.lng, lat: e.lngLat.lat }
        : {}
      void handleCommuneClick({ ...communeFeature.properties, ...clickedPointFallback })
      return
    }
  ...
  const restoreSuiviBanView = useCallback(async () => {
    if (suiviBanSelectedCommune) {
      let coordinates = getCommuneCoordinates(suiviBanSelectedCommune)
      if (!coordinates && suiviBanSelectedCommune?.code) {
        const communeCode = normalizeCommuneCode(suiviBanSelectedCommune.code)
        try {
          const response = await fetch(`${SUIVI_BAN_API}/commune/${encodeURIComponent(communeCode)}`)
          if (response.ok) {
            const full = await response.json()
            coordinates = getCommuneCoordinates(full)
          }
        }
        catch (e) {
          console.warn('Commune restore load error:', communeCode, e)
        }
      }
      if (coordinates) {
        mapRef.current?.flyTo({ center: coordinates, zoom: 13, duration: 600 })
  ...
  }, [suiviBanSelectedCommune, suiviBanSelectedDept, fitToDepartment])
