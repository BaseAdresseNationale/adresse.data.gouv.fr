...
import {
  BaseAdresseLocale,
  BaseAdresseLocaleStatus,
} from "@/types/api-mes-adresses.types";
...
function getSourceInfo(sourceData: CommuneDeploiementData | undefined): {
  label: string;
  color: string;
} {
  if (!sourceData) {
  ...
  if (hasBAL) {
    if (idClient === "mes-adresses")
      return { label: "Mes Adresses", color: toolsColors.mesAdresses };
    if (idClient === "moissonneur-bal")
      return { label: "Moissonneur", color: toolsColors.moissonneur };
    if (idClient === "formulaire-publication")
      return { label: "Formulaire", color: toolsColors.formulaire };
...
export default function CommuneBALList({
  codeCommune,
  balsCommune,
  sourceData,
}: CommuneBALListProps) {
  ...
  const accordionLabel = (
    <span className="accordion-label">
      ...
      <span className="accordion-label-count">
        ...
        {balsCommune.every(
          ({ status }) => status === BaseAdresseLocaleStatus.DRAFT,
        ) && " en Brouillon"}
      </span>
    </span>
  );
    ...
    <StyledWrapper>
      {balsCommune.length > 0 ? (
        ...
        <div className="row-no-bal">
          <span className="accordion-label-left">
            <span className="row-commune-name">
              {codeCommune} {nomCommune}
            </span>
            {sourceBadge}
          </span>
